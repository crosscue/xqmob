# Crosscue Mobility Eventizer Algorithm v1.1

Status: implementation contract for `xqmob` `mobility-reference-v1.1`.
...
`mobility-reference-v1.1` fills that gap for `xqmob` without modifying Core 0.1 or Mobility Profile 0.1. It is a narrow semantic correction to v1: ENTER is only asserted when destination presence completes a previously confirmed departure.
...
Any change that can alter eventization decisions or projection semantics must change `eventizer_algorithm` to a new algorithm identifier/version. `mobility-reference-v1.1` differs from v1 only in the ENTER rule in section 8. Such a change does not, by itself, require a Core Event Model wire-version change.
