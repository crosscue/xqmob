# xqmob analytical contract v0.2

Contract identifier: `xqmob-analysis-v0.2`

Status: reference analytical projection contract for xqmob 0.1.4-rc1; introduced in xqmob 0.1.1.
...
Analytical projection         xqmob-analysis-v0.2
```
...
### Spatial indexing contract

The analytical layer carries both geohash-oriented indexing and H3. Geohash remains the existing human-readable/contextual index. H3 is an additional machine-oriented discrete global grid for aggregation, neighbourhood operations and flows. H3 MUST NOT be used by the reference eventizer to decide movement, presence, gaps or discontinuities.

The stable analytical columns use `h3_cell` names rather than embedding a resolution in the column name. The effective H3 resolution is recorded as `config.h3_resolution` and in Parquet key/value metadata; the reference default is resolution 11. Only the selected resolution is materialized. Coarser parent cells SHOULD be derived downstream rather than stored redundantly. H3 indexes are stored in their standard hexadecimal string representation.
...
Parquet files carry `xq:eventizer`, `xq:analysis_contract`, `xq:h3_resolution`, `xq:h3_library_version`, and `xq:h3_core_version` metadata.
...
The addition of H3 columns is the reason this contract increments from v0.1 to `xqmob-analysis-v0.2`: downstream schemas gain a new spatial-index dimension while eventizer semantics remain unchanged. A future change that redefines the meaning of an existing analytical field, changes support-set semantics, or materially alters layer relationships SHOULD increment the analytical contract identifier independently of the eventizer algorithm.
