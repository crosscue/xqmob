...
This resource describes the current `xqmob-analysis-v0.2` spatial indexing contract. When the MCP server opens an older `xqmob-analysis-v0.1` dataset, geohash remains available but H3 was not materialized. In that compatibility mode the MCP server exposes nullable H3 fields, omits `describe_h3_cell`, and rejects H3-specific filters with an explicit capability error.
